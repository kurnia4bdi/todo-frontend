...
export default {
  ...
  created: function() {
    ...
    axios.get('http://localhost:3080/user', {headers: {username:this.loUsername, password:this.loPassword}})
    .then((result)=>{
  ...
  methods : {
    tambah: function () { 
      ...
      axios.post('http://localhost:3080/user', newUser, {headers: {username:this.loUsername, password:this.loPassword}})
      .then((response) => {
    ...
    hapus: function (id) {
      axios.delete(`http://localhost:3080/user/${id}`, {headers: {username:this.loUsername, password:this.loPassword}})
      .then(()=>{
